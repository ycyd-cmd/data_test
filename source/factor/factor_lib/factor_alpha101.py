...
def get_alpha(df):
        stock=Alphas(df)
        df['alpha', 'alpha001'] = stock.alpha001()
        df['alpha', 'alpha002'] = stock.alpha002()
        df['alpha', 'alpha003'] = stock.alpha003()
        df['alpha', 'alpha004'] = stock.alpha004()
        df['alpha', 'alpha005'] = stock.alpha005()
        df['alpha', 'alpha006'] = stock.alpha006()
        df['alpha', 'alpha007'] = stock.alpha007()
        df['alpha', 'alpha008'] = stock.alpha008()
        df['alpha', 'alpha009'] = stock.alpha009()
        df['alpha', 'alpha010'] = stock.alpha010()
        df['alpha', 'alpha011'] = stock.alpha011()
        df['alpha', 'alpha012'] = stock.alpha012()
        df['alpha', 'alpha013'] = stock.alpha013()
        df['alpha', 'alpha014'] = stock.alpha014()
        df['alpha', 'alpha015'] = stock.alpha015()
        df['alpha', 'alpha016'] = stock.alpha016()
        df['alpha', 'alpha017'] = stock.alpha017()
        df['alpha', 'alpha018'] = stock.alpha018()
        df['alpha', 'alpha019'] = stock.alpha019()
        df['alpha', 'alpha020'] = stock.alpha020()
        df['alpha', 'alpha021'] = stock.alpha021()
        df['alpha', 'alpha022'] = stock.alpha022()
        df['alpha', 'alpha023'] = stock.alpha023()
        df['alpha', 'alpha024'] = stock.alpha024()
        df['alpha', 'alpha025'] = stock.alpha025()
        df['alpha', 'alpha026'] = stock.alpha026()
        df['alpha', 'alpha027'] = stock.alpha027()
        df['alpha', 'alpha028'] = stock.alpha028()
        df['alpha', 'alpha029'] = stock.alpha029()
        df['alpha', 'alpha030'] = stock.alpha030()
        df['alpha', 'alpha031'] = stock.alpha031()
        df['alpha', 'alpha032'] = stock.alpha032()
        df['alpha', 'alpha033'] = stock.alpha033()
        df['alpha', 'alpha034'] = stock.alpha034()
        df['alpha', 'alpha035'] = stock.alpha035()
        df['alpha', 'alpha036'] = stock.alpha036()
        df['alpha', 'alpha037'] = stock.alpha037()
        df['alpha', 'alpha038'] = stock.alpha038()
        df['alpha', 'alpha039'] = stock.alpha039()
        df['alpha', 'alpha040'] = stock.alpha040()
        df['alpha', 'alpha041'] = stock.alpha041()
        df['alpha', 'alpha042'] = stock.alpha042()
        df['alpha', 'alpha043'] = stock.alpha043()
        df['alpha', 'alpha044'] = stock.alpha044()
        df['alpha', 'alpha045'] = stock.alpha045()
        df['alpha', 'alpha046'] = stock.alpha046()
        df['alpha', 'alpha047'] = stock.alpha047()
        df['alpha', 'alpha049'] = stock.alpha049()
        df['alpha', 'alpha050'] = stock.alpha050()
        df['alpha', 'alpha051'] = stock.alpha051()
        df['alpha', 'alpha052'] = stock.alpha052()
        df['alpha', 'alpha053'] = stock.alpha053()
        df['alpha', 'alpha054'] = stock.alpha054()
        df['alpha', 'alpha055'] = stock.alpha055()
        df['alpha', 'alpha057'] = stock.alpha057()
        df['alpha', 'alpha060'] = stock.alpha060()
        df['alpha', 'alpha061'] = stock.alpha061()
        df['alpha', 'alpha062'] = stock.alpha062()
        df['alpha', 'alpha064'] = stock.alpha064()
        df['alpha', 'alpha065'] = stock.alpha065()
        df['alpha', 'alpha066'] = stock.alpha066()
        df['alpha', 'alpha068'] = stock.alpha068()
        df['alpha', 'alpha071'] = stock.alpha071()
        df['alpha', 'alpha072'] = stock.alpha072()
        df['alpha', 'alpha073'] = stock.alpha073()
        df['alpha', 'alpha074'] = stock.alpha074()
        df['alpha', 'alpha075'] = stock.alpha075()
        df['alpha', 'alpha077'] = stock.alpha077()
        df['alpha', 'alpha078'] = stock.alpha078()
        df['alpha', 'alpha081'] = stock.alpha081()
        df['alpha', 'alpha083'] = stock.alpha083()
        df['alpha', 'alpha084'] = stock.alpha084()
        df['alpha', 'alpha085'] = stock.alpha085()
        df['alpha', 'alpha086'] = stock.alpha086()
        df['alpha', 'alpha088'] = stock.alpha088()
        df['alpha', 'alpha092'] = stock.alpha092()
        df['alpha', 'alpha094'] = stock.alpha094()
        df['alpha', 'alpha095'] = stock.alpha095()
        df['alpha', 'alpha096'] = stock.alpha096()
        df['alpha', 'alpha098'] = stock.alpha098()
        df['alpha', 'alpha099'] = stock.alpha099()
        df['alpha', 'alpha101'] = stock.alpha101()
        return df
...
#                             columns=self.close.columns)
    ...
    def alpha023(self):
        ...
        alpha.loc[cond, 'close'] = -1 * delta(self.high, 2).fillna(value=0).loc[cond]
...
#        return (0 - (1 * (rank((sma(self.returns, 10) / sma(sma(self.returns, 2), 3))) * rank((self.returns * self.cap)))))
    ...
    def alpha071(self):
        ...
        df.loc[df['p1'] >= df['p2'], 'max'] = df.loc[df['p1'] >= df['p2'], 'p1']
        df.loc[df['p2'] >= df['p1'], 'max'] = df.loc[df['p2'] >= df['p1'], 'p2']
        return df['max']
    ...
    def alpha077(self):
        ...
        df.loc[df['p1'] >= df['p2'], 'min'] = df.loc[df['p1'] >= df['p2'], 'p2']
        df.loc[df['p2'] >= df['p1'], 'min'] = df.loc[df['p2'] >= df['p1'], 'p1']
        return df['min']
